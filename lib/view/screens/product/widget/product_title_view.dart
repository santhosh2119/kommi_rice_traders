...
class ProductTitleView extends StatelessWidget {
  ...
  final int stock;
  final String amount;
  ProductTitleView({@required this.product, @required this.stock,@required this.amount});
      ...
      child: Consumer<ProductProvider>(
        builder: (context, productProvider, child) {
          return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            //Product Price
          Text("Discount Price :  " + amount,style: poppinsBold.copyWith(
            ...
            Text("Selling Price :  "+
              '${PriceConverter.convertPrice(context, _startingPrice, discount: product.discount, discountType: product.discountType)}'
                  '${_endingPrice!= null ? ' to ${PriceConverter.convertPrice(context, _endingPrice, discount: product.discount, discountType: product.discountType)}' : ''}',
              style: poppinsBold.copyWith(color: ColorResources.getTextColor(context), fontSize: Dimensions.FONT_SIZE_LARGE),
            ),
            product.discount > 0 ? Text("MRP Price :  "+
              '${PriceConverter.convertPrice(context, _startingPrice)}'
                  '${_endingPrice!= null ? ' to ${PriceConverter.convertPrice(context, _endingPrice)}' : ''}',
              style: poppinsRegular.copyWith(color: ColorResources.getHintColor(context), fontSize: Dimensions.FONT_SIZE_SMALL, decoration: TextDecoration.lineThrough),
            ): SizedBox(),
